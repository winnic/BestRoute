...
include "dataLib.php";
    ...
    $SP_buses=SearchAllBusIn($_GET[StartingPoint]);
    $DP_buses=SearchAllBusIn($_GET[DestinationPoint]);
		echo "<script type='text/javascript'>var sp=\"$_GET[StartingPoint]\";var dp=\"$_GET[DestinationPoint]\";console.log(dp);</script>";
...
for($i=0;$i<count($SP_buses);$i++)
    for($j=0;$j<count($DP_buses);$j++){
        if($SP_buses[$i][bus_num]==$DP_buses[$j][bus_num]){
            echo "<p>One of possible buses is: ".$SP_buses[$i][bus_num]."</p>";
            ...
            echo "<tbody><tr><th>Statoin ID</th><th>bus_stationCHI</th><th>accumulate_time</th><th>fee</th></tr>";
			
            for($k=0;$k<count($busInfo);$k++){		
			if(strcmp($_GET[StartingPoint],$busInfo[$k][bus_stationCHI])==0){$color="color:red";}
...
/*			if(((int)$busInfo[$k][station_id]-(int)$IDs[0]>=0)&&(int)$busInfo[$k][station_id]<=8){
                ...
                echo "<tr style=$color><th>".$busInfo[$k][station_id]."</th><th>".$busInfo[$k][bus_stationCHI]."</th><th>".$busInfo[$k][accumulate_time]."</th><th>".$busInfo[$k][fee]."</th></tr>";
			if(strcmp($_GET[DestinationPoint],$busInfo[$k][bus_stationCHI])==0){$color="";}
            }
            echo "</tbody></table>";
			...
			if($accessOnce==1){
				...
				echo "<div id='outputDiv'><button  type='button' onclick='calcDistance();'>Calculate distances</button><//div>";
			$accessOnce=0;
			}
