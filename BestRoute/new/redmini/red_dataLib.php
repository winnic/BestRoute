...
function Insert($bus_num,$station_id,$bus_stationENG,$bus_stationCHI,$district,$lat,$longitube,$accumulate_time,$fee){
    $db=connect_to_bus();
    $q = $db->prepare("insert into ".$tableName."  (bus_num,station_id,bus_stationENG,bus_stationCHI,district,lat,longitube,accumulate_time,fee) values(?,?,?,?,?,?,?,?,?)");
...
function SelectDistrict($point){
    ...
    $db=connect_to_bus();
    $stmt = $db->prepare("select DISTINCT district FROM ".$tableName);
    $stmt->execute();
...
if(isset($_GET['action'])){
	...
	if($_GET['action']=="SelectBusStation"){
		...
		echo "district!!!!!!!!!!!!!!!!!!!!!!";
		print_r($district);
	    $stmt = $db->prepare("select DISTINCT bus_stationCHI from ".$tableName."  where district=?");
...
function Get_Latitiude_Longtitude($start, $end){
	...
	$stmt = $db->prepare("select DISTINCT bus_stationENG, bus_stationCHI, lat, longitube from ".$tableName."  where bus_stationCHI=?");
...
function SearchAllBusIn($position){	
    ...
    $stmt = $db->prepare("select bus_num from ".$tableName."  where bus_stationCHI=?");
    $stmt->execute(array($position));
   // return "select bus_num from ".$tableName."  where bus_stationCHI=".$position;
	return  $stmt->fetchAll(PDO::FETCH_ASSOC);
}
if(isset($_GET['action'])){
		Global $tableName;
	
	if($_GET['action']=="eachBus_stations"){
		$db=connect_to_bus();
		$stmt=$db->prepare("select distinct bus_stationCHI from ".$tableName."  where lat=? and longitube=?");
	...
	if($_GET['action']=="find_LatLng"){
		$db=connect_to_bus();
		$stmt = $db->prepare("select lat, longitube from ".$tableName."  where bus_stationCHI=?");
		if (!$stmt->execute(array($_GET["bus_stationCHI"]))){print_r($stmt->errorInfo());exit;}
	    $result = $stmt->fetch(PDO::FETCH_ASSOC);
			echo json_encode($result);
			return;
	}
	if($_GET['action']=="theNearest"){
		$db=connect_to_bus();
		$stmt = $db->prepare("select bus_stationCHI,lat, longitube from ".$tableName." ");
		if (!$stmt->execute()){print_r($stmt->errorInfo());exit;}
...
function SearchStations_withDtime($bus_num){
		Global $tableName;
    $db=connect_to_bus();
    $stmt = $db->prepare("select bus_stationCHI,accumulate_time from ".$tableName."  where bus_num=?");
...
function SearchBus($bus_num){
		Global $tableName;
    $db=connect_to_bus();
    $stmt = $db->prepare("select * from ".$tableName."  where bus_num=?");
    $stmt->execute(array($bus_num));
    return  $stmt->fetchAll(PDO::FETCH_ASSOC);
}
...
function SearchStations($bus_num){
		Global $tableName;
    $db=connect_to_bus();
    $stmt = $db->prepare("select bus_stationCHI from ".$tableName."  where bus_num=?");
    $stmt->execute(array($bus_num));
    return  $stmt->fetchAll(PDO::FETCH_ASSOC);
} 
/////////////////////////CharToUnicode/////////////////////////////////////////////////////////////
...
if(isset($_GET['action'])){
	if($_GET['action']=="saveWalkingTime"){
