...
include "red_dataLib_redRoute.php";
...
for($i=0;$i<count($SP_buses);$i++)
    for($j=0;$j<count($DP_buses);$j++){
        if($SP_buses[$i]["bus_num"]==$DP_buses[$j]["bus_num"]){
			if($accessOnce==1){
				echo "<a href='function.php'><div id='goBack'><img id='backImg' src='img/back.png' height='38px'/></div></a><h2>".$SP_buses[$i]["bus_num"]."</h2> 		<hr id='menuHr' />   <button id='showMap'>Map</button>";
